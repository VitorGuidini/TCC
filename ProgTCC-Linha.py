...
for circ in circuitos: #cria listas vazias para as variáveis dos circuitos
    ...
    #barWidth = 0.25
    #
    #plt.bar(list_circ_mes, list_circ_total, color="#6A5ACD", width=barWidth, label="Perda (MWh)")
    #plt.bar(list_circ_mes, list_circ_ipts, color="#6395ED", width=barWidth, label="Perda (MWh)")
    ...
    ax.bar(list_circ_mes, list_circ_total, color='red') #gera um gráfico de mês por energia total 
    ax.set_xlabel('Meses', fontsize=14)    
    ax.set_ylabel('Total (MWh)', color='red', fontsize=14) 
    ax2 = ax.twinx() #cria uma segunda escala em um mesmo gráfico 
    ax2.bar(list_circ_mes, list_circ_ipts, color="blue") #gera um gráfico de mês por energia índice de perdas técnicas por segmento 
    ...
    ax3.set_ylabel("perda técnica circui vs Global (%)", color="blue", fontsize=14) 
    ax3.grid() 
    fig1.savefig(f'perda técnica circui vs Global (%)/{circ}.png', dpi=100)   #salva as imagens em uma pasta no computador 
#
