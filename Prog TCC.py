...
for circ in circuitos: #cria listas vazias para as variáveis dos circuitos
    ...
    with open('RedeEstudoTCC.csv', encoding='utf-8-sig') as arq: #carrega o arquivo csv a ser lido
        ...
        for line in reader: 
            ...
            if circ == line[1]: #confere se os circuitos são iguais ao código do csv  
                ...
                list_circ_ei.append(line[4]) # alimennta as listas vazias com as informações da coluna 5
                list_ei_v1= [i.replace('.', '') for i in list_circ_ei] 
                list_ei_v2 = [i.replace(',', '') for i in list_ei_v1]
                list_ei_float = [(float(i)/100) for i in list_ei_v2]
                list_circ_pp.append(float(line[5].replace(',', '.'))) 
    ...
    ax3.plot(list_circ_mes, list_ei_float, color='red', marker='x')  #gera um gráfico de mês por energia injetada
    ax3.set_xlabel('Meses', fontsize=14)
    ax3.set_ylabel('energia injetada (MWh)', color='red', fontsize=14)
    ax4 = ax3.twinx()
    ax4.plot(list_circ_mes, list_circ_pp, color="blue",marker="x") #gera um gráfico de mês por perda própria
    ax4.set_ylabel("perda própria (MWh)", color="blue", fontsize=14) 
    ax3.grid() 
